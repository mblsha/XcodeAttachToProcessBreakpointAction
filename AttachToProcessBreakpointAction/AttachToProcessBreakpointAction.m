...
- (void)attachToProcess:(NSString*)pidString {
  ...
  for (DVTLocalProcessInformation* p in
       [DVTLocalProcessInformation currentProcessInformations]) {
    if ([p pid] == pid) {
      proc = p;
      break;
    }
  ...
  id result = [IDEAttachToProcessHelper attachToProcess:proc
                             fromWorkspaceTabController:workspaceTabController];
  CHECK(result);
}

- (void)performActionUsingContext:(IDEBreakpointActionEvaluationContext*)context
                    andBreakpoint:(IDEBreakpoint*)breakpoint {
...
- (instancetype)initFromXMLUnarchiver:(DVTXMLUnarchiver*)unarchiver
                       archiveVersion:(float)version {
  if ((self =
           [super initFromXMLUnarchiver:unarchiver archiveVersion:version])) {
    [self _debuggerCommandActionCommonInit];
  }
  return self;
}

- (void)dvt_encodeAttributesWithXMLArchiver:(DVTXMLArchiver*)archiver
                                    version:(id)version {
...
- (id)initWithAction:(AttachToProcessBreakpointAction*)action {
  NSBundle* bundle =
      [AttachToProcessBreakpointActionPlugin sharedPlugin].bundle;
  if ((self = [super initWithNibName:@"AttachToProcessBreakpointActionEditor"
                              bundle:bundle])) {
    self.action = action;
  }
  return self;
